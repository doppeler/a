cc.Class({
    ...
    properties: {
        dataEventHandler:null,//从MJGame中initEventHandler传入
        roomId:null,
        maxNumOfGames:0,
        numOfGames:0,
        numOfMJ:0,
    ...
    doChupai:function(seatIndex,pai){
        ...
        var seatData = this.seats[seatIndex];
        if(seatData.holds){             
            var idx = seatData.holds.indexOf(pai);//indexOf方法返回给定元素在数组中第一次出现的位置，如果没有出现则返回-1。
            seatData.holds.splice(idx,1);//splice的第一个参数是删除的起始位置（从0开始），第二个参数是被删除的元素个数。
        }
