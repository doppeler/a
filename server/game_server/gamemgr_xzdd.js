...
function calculateResult(game,roomInfo){
    ...
    //统计杠的数目
        ...
        for(var a = sd.actions.length -1; a >= 0; --a){
            ...
            if(ac.type == "hu"){
                if(sd.pattern == "7pairs"){
                    if(sd.numofgen > 0){                
                        ...
                        game.gameSeats[six].score -= 12;                                                     
                    }           
                    else{
                        score += 9;
                        var six = ac.targets[0];
                        game.gameSeats[six].score -= 9;
                    }
                }
                else{
                    score += 3;           
            ...
            if(ac.type == "zimo" || ac.type == "angang"){
                if(sd.pattern == "7pairs"){
                    if(sd.numofgen > 0){                
                        sd.pattern == "l7pairs";
                        score += 8 * at;                        
        ...
        sd.numMingGang = sd.diangangs.length;
        sd.numWangGan = sd.wangangs.length;
        //对所有胡牌的玩家进行统计
        if(isTinged(sd)){
            //统计自己的番子和分数
    ...
    if(game.qiangGangContext != null){
        ...
        */
        ...
        if(game.fangpaoshumu > 1){
            game.yipaoduoxiang = fs.seatIndex;
        }
    }
